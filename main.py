...
class TicTacToe:
    def __init__(self, game):
        ...
        self.game = game
        self.field_image = self.get_scaled_image(path='images/Background.png', res=[WIN_SIZE] * 2)
        self.O_image = self.get_scaled_image(path='images/O.png', res=[CELL_SIZE] * 2)
        self.X_image = self.get_scaled_image(path='images/X.png', res=[CELL_SIZE] * 2)
    ...
    # ----------------------
    # Static method to load and scale images to the required resolution
    ...
    def print_caption(self):
        ...
        if self.winner:
            ...
            label1 = self.font.render('Game Over! Press Space to restart', True, 'white', 'black')
...
class Game:
    ...
    def check_events(self):
        for event in pg.event.get():
            if event.type == pg.QUIT:  # Check for an exit
                pg.quit()
                sys.exit()
    ...
    def run(self):
        while True:
            ...
            self.check_events()
            pg.display.update()
            self.clock.tick(60)  # Set the number of frames per second
